<template>
    <div class="app-container">
        ...
        <el-dialog v-if="dialogVisible" :title="title" :visible.sync="dialogVisible"  width="80%" >
            <current-step :steps="steps" :value.sync="currentStep" />
        </el-dialog>
    </div>
</template>
...
import YlSteps from '@/components/shared/steps'
...
export default {
  ...
  components: {
    CurrentStep: {
      ...
      render() {
        const { steps, value, handleSubmit } = this
        const { Page } = steps[value - 1]
        return (
          <div>
            <Page submit={handleSubmit}/>
            <yl-steps steps={steps} value={value} />
          </div>
        )
      }
    }
  ...
  methods: {
    ...
    getTableHead() {
      ...
      this.tableHead = localStorage['tableHead']
        ? localStorage['tableHead'].split(',')
        : defaultTableHead
